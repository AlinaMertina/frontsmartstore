...
export default function Pagination({setData,size,debut,fin}){
    const [debut, setDebut] = useState(0);
    const suivant = async (e) => {
        e.preventDefault();
        const newDebut = debut + size;
        setDebut( newDebut);
        console.log(config.lienCrudmetier+'date_exceptionnels/page/'+newDebut+'/'+size+'/'+debut+'/'+fin)
        axios.get(config.lienCrudmetier+'date_exceptionnels/page/'+newDebut+'/'+size+'/'+debut+'/'+fin)
          .then(response => {
